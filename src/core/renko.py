...
class LiveRenkoState:
    ...
    def process_tick(
        ...
        # Fix: Normalize timestamp to naive to prevent "can't compare offset-naive and offset-aware" errors
        ...
        if abs(move) >= self.brick_size:
            ...
            # THE FIX: Distribute the accumulated volume evenly across all bricks generated by this tick
            vol_per_brick = self._brick_volume / bricks_to_generate
            ...
            while abs(move) >= self.brick_size:
                ...
                brick = {
                    ...
                    "volume":           vol_per_brick,  # Use the distributed volume
                    "typical_price":    typical,
                    "cum_volume":       0.0,
                    "true_gap_pct":     self._current_candle_gap_pct,
                    "time_to_form_seconds": min(3600.0, (timestamp - self._last_brick_timestamp).total_seconds()) if self._last_brick_timestamp else 0.0,
                    "volume_intensity_per_sec": vol_per_brick / max(1.0, min(3600.0, (timestamp - self._last_brick_timestamp).total_seconds()) if self._last_brick_timestamp else 0.0),
                    "is_opening_drive": 1 if time(9, 15) <= timestamp.time() <= time(10, 0) else 0
                }
                self.bricks.append(brick)
                new_bricks.append(brick)
                ...
                # CRITICAL: Removed self._brick_volume = 0.0 from inside this loop

            self.brick_start_time = timestamp
            
            # Reset the total accumulator strictly AFTER all bricks are generated
            self._brick_volume = 0.0
