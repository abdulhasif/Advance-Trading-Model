...
_HEADERS = [
    ...
    "score", "global_kill", "global_pause", "ticker_paused", "bias", "eff_prob_thresh",
    ...
    def run(self):
        while True:
            try:
                ...
                row = [ts_dt.strftime("%Y-%m-%d %H:%M:%S")] + list(event[1:])
...
def log_brick_event(
    ...
    **kwargs # Capture any extra gate audits
    ...
    event = (
        ...
        signal, round(score, 2), int(global_kill), int(global_pause),
